function generatePages() {
  ...
  pages.forEach((page, index) => {
    let numDiv = page.querySelector(".pagenum");
    numDiv.textContent = `Page ${index + 1}`;
  });
}
...
async function downloadPDF() {
  ...
  // Dynamic file name
  const place = document.getElementById("place").value || "Report";
  const year = document.getElementById("year").value || new Date().getFullYear();
  const fileName = `${place}-${year}.pdf`;
