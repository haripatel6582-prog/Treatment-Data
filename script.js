function generatePages() {
  ...
  // Header HTML for each page
  function pageHeader(){
    return `<div class="header">
      <img src="gujarat_logo.png" alt="Gujarat Logo" style="float:left;">
      <div class="title">મહિસાગર વન વિભાગ, લુણાવાડા<br><span style='font-size:16px; font-weight:normal;'>વડોદરા વન વર્તુળ, વડોદરા</span></div>
      <img src="forest_logo.png" alt="Forest Logo" style="float:right;">
  ...
  for (let i = 1; i <= blockCount; i++) {
    let dropdown = `<select>`;
    symbols.forEach(sym => { dropdown += `<option value='${sym}'>${sym}</option>`; });
    dropdown += `</select>`;
    tableRows += `<tr><td>બ્લોક - ${i}</td><td>${dropdown}</td><td><input type='text' placeholder='વિગત લખો'></td></tr>`;
  }
  ...
  outputHTML += `
    <div class="page">
      ...
      ${imageBox}
      <table>
        <thead><tr><th>બ્લોક નંબર</th><th>ચિન્હ</th><th>વિગત</th></tr></thead>
